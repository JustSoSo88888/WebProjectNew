//巴西语
let pt = {
    "登录": "Entrar",
    "注册": "Cadastrar-se",
    "欢迎开启您的专属旅程": "Bem-vindo à sua jornada exclusiva",
    "管理流量与收益，继续增长": "Gerencie tráfego e ganhos em um só lugar",
    "创建账户，开始提升收益": "Crie uma conta e comece a aumentar seus ganhos",
    "让增长更直接": "Tráfego inteligente. Crescimento real.",
    "全球流量": "Alcance global",
    "收益增长": "Pagamentos rápidos",
    "手机号": "Número de telefone",
    "密码": "Senha",
    "没有账户": "Não tem uma conta?",
    "已有账户": "Já tem uma conta?",
    "请输入手机号": "Por favor, insira seu número de telefone",
    "请输入密码": "Por favor, insira sua senha",
    "请输入6到18位密码": "Por favor, insira uma senha entre 6 e 18 caracteres",
    "登录成功": "Login realizado com sucesso",
    "今天就开始您的专属旅程吧": "Comece sua jornada exclusiva hoje",
    "请输入确认密码": "Por favor, insira a confirmação da senha",
    "密码不一致请重新输入": "As senhas não coincidem, por favor insira novamente",
    "请输入邀请码": "Por favor, insira o código de convite",
    "邀请码": "Código de convite",
    "确认密码": "Confirmar senha",
    "添加到主屏幕，获得更好体验": "Adicione à tela inicial para uma melhor experiência",
    "安装": "Instalar",
    "建议使用Chrome": "Recomendado: usar o Chrome",
    "使用Chrome浏览器以获得更好体验": "Use o navegador Chrome para uma melhor experiência",
    "点击底部": "Toque na parte inferior",
    "分享按钮": "Botão de compartilhamento",
    "选择「添加到主屏幕」": "Selecione \"Adicionar à tela inicial\"",
    "点击「添加」确认": "Toque em \"Adicionar\" para confirmar",
    "添加到主屏幕": "Adicionar à tela inicial",
    "注册成功": "Cadastro realizado com sucesso",
    "知道了": "Entendi",
    "选择语言": "Selecionar idioma",
    "提示": "Aviso",
    "登录超时，请重新登录": "Sessão expirada, por favor faça login novamente",
    "请先绑定银行卡": "Por favor, vincule um cartão bancário primeiro",
    "确认": "Confirmar",
    "取消": "Cancelar",
    "确认退出登录？": "Tem certeza de que deseja sair?",
    "退出成功": "Saída realizada com sucesso",
    "加载中": "Carregando",
    "请设置交易密码": "Por favor, defina uma senha de transação",
    "设置交易密码": "Definir senha de transação",
    "请设置6位数字交易密码": "Por favor, defina uma senha de transação de 6 dígitos",
    "交易密码": "Senha de transação",
    "提交中": "Enviando",
    "确认设置": "Confirmar configuração",
    "请输入6位支付密码": "Por favor, insira uma senha de pagamento de 6 dígitos",
    "支付密码设置成功": "Senha de pagamento definida com sucesso",
    "两次输入的密码不一致": "As senhas inseridas não coincidem",
    "首页": "Inicio",
    "理财": "Finanzas",
    "团队": "Equipo",
    "任务": "Tareas",
    "个人中心": "Perfil",
    "退出登录": "Cerrar sesión",
    "语言": "Idioma",
    "设置": "Configuración",
    "旧密码": "Senha antiga",
    "新密码": "Nova senha",
    "确认新密码": "Confirmar nova senha",
    "修改登录密码": "Alterar senha de login",
    "修改交易密码": "Alterar senha de transação",
    "请输入旧密码": "Por favor, insira sua senha antiga",
    "请输入新密码": "Por favor, insira uma nova senha",
    "请输入确认新密码": "Por favor, confirme sua nova senha",
    "新密码与确认密码不一致": "A nova senha e a confirmação não coincidem",
    "交易密码必须为6位数字": "A senha de transação deve ter 6 dígitos",
    "支付设置": "Configurações de pagamento",
    "银行卡设置": "Configurações do cartão bancário",
    "安全设置": "Configurações de segurança",
    "密码修改成功": "Senha alterada com sucesso",
    "银行卡信息": "Informações do Cartão Bancário",
    "姓名": "Nome",
    "请输入姓名": "Por favor, insira o nome",
    "银行账户": "Conta Bancária",
    "请选择银行账户": "Por favor, selecione uma conta bancária",
    "账户号码": "Número da Conta",
    "请输入账户号码": "Por favor, insira o número da conta",
    "编辑": "Editar",
    "提交": "Enviar",
    "交易记录": "Histórico de Transações",
    "现金礼物": "Presente em Dinheiro",
    "请输入兑换码领取现金奖励": "Por favor, insira o código de resgate para receber recompensas em dinheiro",
    "请输入兑换码": "Por favor, insira o código de resgate",
    "立即兑换": "Resgatar Agora",
    "查看详情": "Ver Detalhes",
    "兑换成功": "Resgate Concluído",
    "我知道了": "Entendi",
    "现金礼物记录": "Histórico de Presentes em Dinheiro",
    "没有更多了": "Não há mais dados",
    "暂无数据": "Nenhum dado disponível",
    "下拉即可刷新": "Puxe para atualizar",
    "释放即可刷新": "Solte para atualizar",
    "兑换码": "Código de resgate",
    "奖励": "Recompensa",
    "收益": "Ganhos",
    "新闻": "Notícias",
    "关于我们": "Sobre nós",
    "手册": "Manual",
    "帮助": "Ajuda",
    "总充值": "Depósito total",
    "总提现": "Saque total",
    "注册时间": "Data de registro",
    "团队详情": "Detalhes da equipe",
    "幸运转盘": "Roda da Sorte",
    "剩余次数": "Tentativas Restantes",
    "抽奖中": "Sorteando",
    "开始": "Começar",
    "中奖记录": "Registros de Ganhos",
    "用户": "Usuário",
    "时间": "Hora",
    "恭喜获得奖励": "Parabéns, você ganhou uma recompensa",
    "收下奖励": "Receber Recompensa",
    "记录": "Registros",
    "我的奖励": "Minhas Recompensas",
    "在线": "Online",
    "连接中": "Conectando",
    "离线": "Offline",
    "任务中心": "Central de Tarefas",
    "今日进度": "Progresso de Hoje",
    "完成广告任务后，奖励将自动进入账户": "Após concluir as tarefas de anúncios, as recompensas serão creditadas automaticamente na sua conta",
    "任务图片": "Imagem da tarefa",
    "进行中": "Em andamento",
    "已完成": "Concluído",
    "全部": "Todos",
    "收益": "Ganhos",
    "完成": "Concluído",
    "立即领取": "Receber agora",
    "恭喜获得": "Parabéns",
    "奖励已发放到您的账户": "A recompensa foi creditada na sua conta",
    "任务详情": "Detalhes da Tarefa",
    "图片": "Image",
    "客服": "Atendimento ao Cliente",
    "条新消息": "novas mensagens",
    "请上传 .png, .jpg, .jpeg 或 .gif 格式的图片": "Por favor, envie imagens nos formatos .png, .jpg, .jpeg ou .gif",
    "您的发言频率过快": "Você está enviando mensagens com muita frequência",
    "发送": "Enviar",
    "我的团队": "Minha equipe",
    "团队总人数": "Total de membros da equipe",
    "团队收入": "Renda da equipe",
    "团队规模": "Tamanho da equipe",
    "人": "Pessoas",
    "注册人数": "Número de registrados",
    "任务收入": "Renda de tarefas",
    "推荐收入": "Renda de indicação",
    "升级即可获得特权和奖励。": "Atualize para obter privilégios e recompensas.",
    "小型": "Pequeno",
    "中型": "Médio",
    "大型": "Grande",
    "邀请": "Convidar",
    "A团队": "Equipe A",
    "B团队": "Equipe B",
    "C团队": "Equipe C",
    "复制成功": "Copiado com sucesso",
    "复制失败，请手动复制": "Falha ao copiar, copie manualmente",
    "信用评分": "Pontuação de crédito",
    "理财记录": "Registros de investimento",
    "余额": "Saldo",
    "总收益": "Rendimento total",
    "今日收益": "Rendimento de hoje",
    "理财产品": "Produtos financeiros",
    "天": "Dias",
    "日收益率": "Taxa de retorno diária",
    "最低存款": "Depósito mínimo",
    "扫码邀请好友": "Escaneie o QR Code para convidar amigos",
    "邀请好友加入，获得丰厚推荐奖励": "Convide amigos para participar e ganhe generosas recompensas por indicação",
    "复制": "Copiar",
    "已复制": "Copiado",
    "邀请链接": "Link de convite",
    "优惠券": "Cupom",
    "有效期": "Período de validade",
    "至": "Até",
    "可使用": "Disponível",
    "产品详情": "Detalhes do produto",
    "天数": "Dias",
    "日收益率": "Taxa de retorno diária",
    "最低金额": "Valor mínimo",
    "产品简介": "Descrição do produto",
    "购买理财": "Comprar investimento",
    "投资金额": "Valor do investimento",
    "最低投资金额": "Valor mínimo de investimento",
    "请输入投资金额": "Por favor, insira o valor do investimento",
    "选择优惠券": "Selecionar cupom",
    "已优惠": "Desconto aplicado",
    "优惠": "Desconto",
    "实付金额": "Valor pago",
    "预计总收益": "Rendimento total estimado",
    "立即投资": "Investir agora",
    "不使用优惠券": "Não usar cupom",
    "投资成功": "Investimento bem-sucedido",
    "您的理财产品已购买成功，请耐心等待收益": "Seu produto de investimento foi adquirido com sucesso, aguarde os rendimentos",
    "查看理财记录": "Ver registros de investimento",
    "最低投资金额为": "O valor mínimo de investimento é",
    "确认充值": "Confirmar depósito",
    "充值方式": "Método de depósito",
    "输入其他金额": "Inserir outro valor",
    "充值金额": "Valor do depósito",
    "当前余额": "Saldo atual",
    "充值": "Depositar",
    "支付方式": "Pagamento",
    "充值渠道": "Canais de depósito",
    "立即充值": "Depositar agora",
    "赠送": "bônus",
    "推荐": "RECO",
    "请联系客服进行充值。": "Entre em contato com o atendimento ao cliente para fazer um depósito.",
    "成为广告主": "Tornar-se anunciante",
    "值得信赖的广告网络": "Uma rede de publicidade confiável",
    "与可靠的合作伙伴共赢增长，带来看得见的收益": "Cresça com parceiros confiáveis e resultados visíveis",
    "购买全球广告流量，精准触达来自巴基斯坦、印度、印尼、菲律宾、孟加拉国、巴西、墨西哥、美国等248地区的线上目标人群": "Compre tráfego publicitário global e alcance públicos online em 248 regiões, incluindo Paquistão, Índia, Indonésia, Filipinas, Bangladesh, Brasil, México e Estados Unidos.",
    "Adsterra 助力合作伙伴实现长期收益增长": "A Adsterra ajuda parceiros a aumentar ganhos no longo prazo",
    "稳定且高质量的流量": "Tráfego estável e de alta qualidade",
    "依托全球直接优质媒体，提供规模化、真实有效的高质量流量": "Acesse tráfego escalável e real de publishers premium diretos no mundo todo.",
    "成本优化": "Otimização de custos",
    "强大的流量筛选工具和智能设置，精准控制广告成本，锁定高效流量": "Use filtros avançados e configurações inteligentes para controlar custos e focar no tráfego eficiente.",
    "广告与流量安全": "Segurança de anúncios e tráfego",
    "独家三重安全防护系统，有效防御欺诈与恶意软件": "Um sistema próprio de proteção em três níveis ajuda a combater fraude e malware.",
    "透明且高比例的结算率": "Taxas transparentes e competitivas",
    "优质发布商持续变现，受益于极具竞争力的 CPM 价格": "Publishers premium monetizam continuamente com preços CPM altamente competitivos.",
    "用户友好的广告格式": "Formatos de anúncio amigáveis",
    "针对移动操作系统优化的高性能广告，持续提供相关广告推送": "Anúncios de alto desempenho otimizados para mobile entregam promoções relevantes em todos os dispositivos.",
    "尊享支持": "Suporte dedicado",
    "合作伙伴关怀计划包含专业的多语言支持": "O programa de atendimento a parceiros inclui suporte profissional multilíngue.",
    "广告主": "Anunciante",
    "发布商": "Publisher",
    "查看资料包，获取更多信息。": "Veja o media kit para obter mais informações.",
    "始于 2013，全面助力您的增长": "Apoiando seu crescimento desde 2013",
    "自 2013 年起，Adsterra 通过领先的广告投放与流量变现方案，持续助力全球广告主与发布商增长收益。这是一个让每一位合作伙伴都能实现清晰且卓越的增长目标的共赢平台。": "Desde 2013, a Adsterra ajuda anunciantes e publishers globais a aumentar ganhos com soluções avançadas de compra de anúncios e monetização de tráfego. É uma plataforma ganha-ganha para que cada parceiro alcance metas de crescimento claras e excelentes.",
    "多样化的广告格式，契合您的目标": "Formatos de anúncio variados para seus objetivos",
    "简洁且友好的浏览器通知": "Notificações de navegador simples e amigáveis",
    "观看演示": "Ver demonstração",
    "立即加入 Adsterra": "Entre na Adsterra agora",
    "为您的营销添加高收益流量来源，或开始从您的网站或社交流量中赚取更多收益。": "Adicione uma fonte de tráfego de alto rendimento ao seu marketing ou comece a ganhar mais com seu site ou tráfego social.",
    "购买优质流量": "Comprar tráfego de qualidade",
    "开始流量变现": "Começar a monetizar tráfego",
    "36,000+ 优质发布商持续变现，受益于极具竞争力的 CPM 价格": "Mais de 36.000 publishers premium monetizam continuamente com preços CPM altamente competitivos.",
    "针对移动操作系统优化的高性能广告，100%提供相关广告的推送": "Anúncios de alto desempenho otimizados para sistemas móveis entregam anúncios relevantes em 100% das exibições.",
    "独特的 Partner Care Program (合作伙伴关怀计划) 包含专业的多语言支持。": "O exclusivo Partner Care Program inclui suporte profissional multilíngue.",
    "取代传统流量推送的最先进替代方案": "A alternativa mais avançada ao tráfego push tradicional",
    "无需用户许可，广告可在所有操作系统和浏览器上运行。": "Não exige permissão do usuário, e os anúncios funcionam em todos os sistemas operacionais e navegadores.",
    "CPM, Smart CPM, CPA Goal 或者 CPA Pricing": "CPM, Smart CPM, CPA Goal ou CPA Pricing",
    "广告优惠会在新的浏览器标签页中打开。": "As ofertas de anúncios abrem em uma nova aba do navegador.",
    "每月118亿次广告展示": "11,8 bilhões de impressões de anúncios por mês",
    "广告直接投放给高意向受众。": "Os anúncios são entregues diretamente para públicos com alta intenção.",
    "用户体验友好的广告可在移动和桌面浏览器上运行。": "Anúncios amigáveis à experiência do usuário funcionam em navegadores móveis e desktop.",
    "非侵入式广告，月均9.9亿次展示。": "Anúncios não intrusivos com 990 milhões de impressões mensais.",
    "专有创意广告，点击率高达30%。": "Criativos proprietários com CTR de até 30%.",
    "成本": "Custo",
    "总天数": "Total de dias",
    "有效期": "Período de validade",
    "每日收益率": "Taxa de retorno diária",
    "有效期": "Período de validade",
    "理财记录": "Registros de investimento",
    "信用分": "Pontuação de crédito",
    "有效日期": "Data de validade",
    "收益统计": "Estatísticas de ganhos",
    "昨日收入": "Ganhos de ontem",
    "今日收入": "Ganhos de hoje",
    "7日收入": "Ganhos de 7 dias",
    "本月收入": "Ganhos deste mês",
    "团队任务收入": "Ganhos de tarefas da equipe",
    "团队邀请收入": "Ganhos de indicação da equipe",
    "总收入": "Ganhos totais",
    "提现": "Saque",
    "提现驳回": "Saque rejeitado",
    "提现失败": "Falha no saque",
    "团队收益": "Ganhos da equipe",
    "理财收益": "Ganhos de investimento",
    "理财到期": "Vencimento do investimento",
    "理财收益结算": "Liquidação dos ganhos de investimento",
    "升级": "Atualizar",
    "购买理财": "Comprar investimento",
    "任务收益": "Ganhos de tarefas",
    "转盘收益": "Ganhos da roleta",
    "注册赠送": "Bônus de registro",
    "银行": "Banco",
    "请输入银行": "Por favor, insira o banco",
    "银行卡号": "Número do cartão bancário",
    "请输入银行卡号": "Por favor, insira o número do cartão bancário",
    "银行名称": "Nome do banco",
    "请输入银行名称": "Por favor, insira o nome do banco",
    "用户名称": "Nome do usuário",
    "请输入用户名称": "Por favor, insira o nome do usuário",
    "类型": "Tipo",
    "请选择银行类型": "Por favor, selecione o tipo de banco",
    "请联系客服更改": "Por favor, entre em contato com o atendimento ao cliente para alterar",
    "绑定成功": "Vinculação bem-sucedida",
    "确认要升级？": "Tem certeza de que deseja atualizar?",
    "升级成功": "Atualização bem-sucedida",
    "立即加入": "Junte-se agora",
    "价格": "Preço",
    "任务": "Tarefas",
    "任务奖励": "Recompensas de tarefas",
    "会员等级": "Nível de associação",
    "当前等级": "Nível atual",
    "等级说明": "Explicação do nível",
    "****9000邀请3级新用户佣金：100 ABCD": "****9000 Convite LV3 Bônus de comissão 100",
    "手续费": "Taxa de transação",
    "最低提现金额": "Valor mínimo de saque",
    "选择或输入提现金额": "Selecione ou insira o valor do saque",
    "账户余额": "Saldo da conta",
    "流水要求": "Requisito de rollover",
    "提现方式": "Método de saque",
    "提现金额": "Valor do saque",
    "最大": "Máx",
    "最高": "Máx",
    "提现账户": "Conta de saque",
    "我的账户": "Minha conta",
    "更改": "Alterar",
    "添加账户": "Adicionar conta",
    "请添加用于提现的账户": "Adicione uma conta para saque.",
    "账户": "Conta",
    "请输入钱包账户": "Insira a conta da carteira",
    "请输入银行账户": "Insira a conta bancária",
    "请输入CNIC": "Insira o CNIC",
    "请确保提现地址信息与本人信息一致": "Certifique-se de que as informações da conta de saque correspondem às suas informações.",
    "信息保存后将用于后续提现": "As informações salvas serão usadas para saques futuros.",
    "请添加提现地址": "Adicione um endereço de saque",
    "提现金额不能大于余额": "O valor do saque não pode exceder o saldo",
    "您已有提现订单处理中": "Você já tem um pedido de saque em andamento",
    "提现中": "Sacando",
    "保存成功": "Salvo com sucesso",
    "立即提现": "Sacar agora",
    "实际到账金额": "Valor líquido recebido",
    "请输入金额": "Por favor, insira um valor",
    "确认提现": "Confirmar saque",
    "提现申请已提交": "Solicitação de saque enviada",
    "您的提现申请已提交，请耐心等待": "Sua solicitação de saque foi enviada. Por favor, aguarde pacientemente.",
    "财务记录": "Histórico Financeiro",
    "单号": "Número do pedido",
    "金额": "Valor",
    "状态": "Status",
    "已到达": "Concluído",
    "不通过": "Rejeitado",
    "线上打款中": "Pagamento em processamento",
    "用户取消": "Cancelado pelo usuário",
    "未审核": "Pendente de revisão",
    "待支付": "Pendente de pagamento",
    "已支付": "Pago",
    "已取消": "Cancelado",
    "交易失败": "Falha na transação",
    "请输入消息内容": "Por favor, insira sua mensagem",
    "选择银行卡": "Selecionar cartão bancário",
    "跳转": "Ir para",
    "Telegram客服": "Atendimento ao cliente do Telegram",
    "别错过这个！": "Não perca essa! ",
    "带上你的VIP朋友，让你离大奖越来越近。你的中奖机会随每一次邀请而成倍增长！": "Traga seus amigos VIP e fique cada vez mais perto do Grande Prêmio. Suas chances se multiplicam com cada convite!",
    "请点击确认前往支付页面": "Clique em Confirmar para ir para a página de pagamento",
    "您即将跳转至支付页面": "Você será redirecionado para a página de pagamento",
    "只有VIP用户可提现,请前往充值页充值！": "Somente usuários VIP podem sacar fundos. Acesse a página de recarga para recarregar!",
    "卡号": "Número do Cartão",
    "请输入卡号": "Por favor, insira o número do cartão",
    "邮箱": "Email",
    "请输入邮箱": "Por favor, insira o email",
    "手机号(11 位数字，仅限数字)": "Número de telefone (11 dígitos, apenas números)",
    "请输入手机号(11 位数字，仅限数字)": "Por favor, insira o número de telefone (11 dígitos, apenas números)",
    "只有VIP用户可提现,请联系客服！":"Apenas usuários VIP podem realizar saques. Entre em contato com o atendimento ao cliente!",

}

export default pt;
